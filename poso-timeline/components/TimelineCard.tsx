...
export default function TimelineCard({ event, position }: Props) {
  ...
  /* ── Year + title block — always visible ── */
  const Header = (
    <div style={{ padding: "12px 12px 8px 16px" }}>
      ...
      <p style={{
        ...
        color: "#E94B3F",
        lineHeight: 1.1,
        marginBottom: event.date ? "2px" : "6px",
      ...
      {/* Key event title */}
      <h3 style={{ fontFamily: "var(--font-hanken)", fontSize: "18px", fontWeight: 700, lineHeight: 1.3, color: "#1a1208" }}>
        {event.keyEvent}
      </h3>
    </div>
  );

  /* ── Description block — scrollable, only when expanded ── */
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          boxShadow: pinned ? `0 8px 32px 0 ${colors.dot}44` : expanded ? `0 4px 20px 0 ${colors.dot}28` : `0 1px 6px 0 ${colors.dot}18`,
          transition: "box-shadow 0.2s ease, border-color 0.2s ease",
          cursor: "pointer",
          display: "flex",
          flexDirection: position === "above" ? "column-reverse" : "column", // ← key fix
        }}
